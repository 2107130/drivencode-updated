...
<!-- Blog Area Start -->
<section class="blog--layout1 bg-smoke space-top space-extra-bottom">
  <div class="container">
    <div class="row align-items-end">
      <div class="col-lg-12 col-auto">
        <div class="title-area">
          <span class="sec-subtitle style2">Our Coded Cars</span>
        </div>
      </div>
    </div>
    <div class="row" id="codedCarsContainer">
      <?php foreach ($codedCars as $car): ?>
        <?= view('partials/coded_car_card', ['car' => $car]) ?>
      <?php endforeach; ?>
    </div>
...
<script>
  document.getElementById('loadMoreBtn')?.addEventListener('click', function() {
    ...
    fetch(`<?= base_url('load-more-coded-cars') ?>?offset=${offset}`)
      .then(res => res.json())
      .then(cars => {
        if (cars.length > 0) {
          const container = document.getElementById('codedCarsContainer');
            ...
            col.innerHTML = `
              <div class="vs-blog style3">
                <div class="blog-img" style="background: #ededf1;">
                  <a href="#"><img class="audi q7.jpeg" src="<?= base_url('uploads/coded_cars/') ?>${car.coded_car_image}" alt="${car.coded_car_name}"></a>
                  <div class="vs-blog__meta">
                    <span class="blog-date">
                      ${new Date(car.coded_car_date).getDate()}
                      <span>${new Date(car.coded_car_date).toLocaleString('default', { month: 'short', year: 'numeric' })}</span>
                    </span>
                  </div>
                </div>
                <div class="blog-content" style="padding: 10px;">
                  <p>${car.coded_car_description}</p>
                </div>
              </div>
            `;
          ...
          btn.setAttribute('data-offset', offset + cars.length);
